...
$(function () {
  ...
  var moveToPage = function (_index, _next){
    if(!canMove){
      return false;
    }
    ...
    // 如果有视频,就暂停
    $pages.eq(pageIndex).find('video').addClass('hide')[0].pause();
  ...
  // 圆圈点击关闭
  $('.share-box .btn-close').on('tap', function () {
    $(this).closest('.shade-content').removeClass('anm').addClass('out-anm');
    if (pageIndex == shareIndex) {
      setTimeout(function () {
        moveToPage(0, false);
      }, 300);
    } else {
  ...
  $('.page-like .btn-like').on('tap', function (event, isDisable) {
    ...
    if (!isDisable) {
      // 点赞完成自动move到下一页
      setTimeout(function (){
        $(ctx).closest('.share-box').find('.btn-close').trigger('tap');
      }, 400);
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var key in data.response) {
          ...
          if(like == thread.thread_key){
            $('.page-like .btn-like').trigger('tap', [window.event, true]);
          }
        }
